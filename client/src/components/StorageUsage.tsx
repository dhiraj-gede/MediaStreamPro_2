...
import { AccountsApiResponse } from "@/pages/Storage";
...
export const StorageUsage: React.FC<StorageUsageProps> = ({ limit = 3 }) => {
  ...
  const totalStorageLimit = accounts?.accounts?.reduce(
    (sum, account) => sum + account.storageLimit,
    0
  ) ?? 0;
  const totalStorageUsed = accounts?.accounts?.reduce(
    (sum, account) => sum + account.storageUsed,
    0
  ) ?? 0;
  const overallUsagePercentage =
    totalStorageLimit > 0
      ...
      : 0;
  const displayedAccounts = limit ? accounts?.accounts?.slice(0, limit) : accounts?.accounts ?? [];
  ...
  return (
    <Card>
      <CardHeader className="flex flex-col md:flex-row md:items-center justify-between pb-2">
        <CardTitle>Storage Usage</CardTitle>
        {limit && accounts.accounts && accounts.accounts.length > limit && (
          <Link
            href="/storage"
                ...
                <div className="space-y-2">
                  <div>
                    ...
                    <Progress
                      ...
                      className={`h-2 ${isLowSpace ? "bg-yellow-200 [&>div]:bg-yellow-500" : ""}`}
                    />
                  </div>
                </div>
              </div>
